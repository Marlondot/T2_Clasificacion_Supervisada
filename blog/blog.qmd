...
Para realizar el siguiente reporte utilizamos la base de datos facilitada por @DataWebsite que además otorga un diccionario con las definiciones específicas de cada una de las variables

### Contexto del problema

Una de las principales actividades de los bancos a lo largo de la historia es el prestamo de dinero o la aplicación de créditos a personas. Dicha actividad conlleva un gran reto para estas: conocer criterios desde los cuales puedan concluir que el dinero prestado más los intereses sí se les sea dado en el plazo estipulado.

¿Por qué es importante saber si el prestatario devolverá el dinero? Minimizar el impacto y la tasa de cartera vencida. Entiendase por cartera vencida por créditos no pagados a tiempo. Para mediados de junio la cartera de bancos extranjeros fue del 3.2% y en Colombia el indicador de cartera vencida fue del 3.7% [@PagNews1]. No tener en cuenta este factor puede provocar que toda la economía de un país colapse. Como ocurrió en la recesión del 2008 donde los bancos hicieron prestamos a personas con altas probabilidadesde incumplimiento [@PagNews2].

### Resumen resultados

Al final del reporte se podrá notar que una de las variables más importantes para la puntuación en la score card generada es 'grade', donde un alto grado (A)puede sumar una gran puntuación, mientras que un grado bajo (F) puede por el contrario ser de los atributos que más efectos negativos tengan sobre la puntuación general. Por otra parte se puede notar que a menor ingresos anuales declarados menor es la puntuación otorgada.

### Metodos empleados y objetivos de desarrollo

Teniendo en cuenta el contexto anterior en el siguiente trabajo se desarollará una scorecard de riesgo crediticio, basados en técnicas de aprendizaje estadístico, para así obtener la probabilidad de que un prestatario pague el dinero prestado. Basándonos en el proceso seguido por @Data1, y apoyados en diferentes bibliografías en materia de Machine Learning y Credit Scoring, se usará Google colab y Python junto con la librería sklearn, además del framework streamlit para la creación de un aplicativo web que simplifique la consulta de Credit Score a partir de diferentes variables.
...
El WOE y IV son criterios de selección que se usan en el campo de evaluación de riesgos y scorecards. WOE, o Weight of Evidence, se usa para medir la fuerza predictiva de un atributo o grupo de atributos, siendo esta una medida de la diferencia entre la proporción de "buenos" y "malos" en cada atributo, por ejemplo la probabilidad de que una persona con dicho atributo sea buena o mala. Así, este se calcula con el logaritmo natural de la división de la proporción de observaciones buscadas ("buenas") sobre las no buscadas ("malas") dada la cantidad de observaciones de cada una de las labels del atributo [@Creditriskscorecards]. Por otra parte el IV, o Information Value, se mide como la sumatoria total de la proporción de observaciones "buenas" de la label i menos la proporción de observaciones "malas" de la label i por el valor del WOE calculado de la label i, con i perteneciente al conjunto de todas las labels posibles del atributo [@Creditriskscorecards], su función es determinar cuales agrupamientos realizados en el WOE tienen mayor fuerza predictiva, así dado el valor se tienen las posibles decisiones:

*   Valores IV menores a 0.02: No mantener atributos pues no aportan a la predicción
*   Valores IV desde 0.02 a 0.1: Mantener atributos pues aportan a la predicción de manera débil
*   Valores IV desde 0.1 a 0.3: Mantener atributos pues aportan a la predicción de manera media
*   Valores IV desde 0.3 a 0.5: Mantener atributos pues aportan a la predicción de manera fuerte
*   Valores IV mayores a 0.5: Revisar atributos y mantenerlos con cautela pues están sobre prediciendo [@Creditriskscorecards]

Antes de comenzar con la selección haremos imputación a los datos:
...
*   'purpose': El propósito por el que se pide el crédito
*   'verification_status': Indica si los ingresos fueron verificados por el Lending Club
*   'home_ownership': El estado actual de propiedad de la vivienda del prestatario
*   'tot_cur_bal': El balance actual total de todas las cuentas
*   'dti': Una relación calculada usando los pagos totales de deuda mensuales del prestatario sobre el total de obligaciones de deuda, sin incluir la hipoteca y el préstamo de LC solicitado, dividido por el total de ingresos mensuales autodeclarados del prestatario
*   'inq_last_6mths': Número de consultas en los últimos 6 meses (excluyendo consultas de hipotecas)
*   'out_prncp': Capital restante pendiente por el monto total financiado
*   'int_rate': Tasa de interés en el préstamo
*   'term': Número de pagos al préstamo. Sólo toma dos valores: 36 meses o 60 meses
*   'annual_inc': Los ingresos anuales autodeclarados del prestatario
*   'revol_util': Tasa de utilización del préstamo
*   'grade': Grado del préstamo asginado en la carta de crédito
...
Como se puede ver en la matriz de confusión, @fig-matriz-conf:

- Arriba a la izquierda el porcentaje de los verdaderos negativos clasificados por el modelo.
- Arriba a la derecha el porcentaje de los falsos positivos que clasificó el modelo.
- Abajo a la izquierda el porcentaje de los falsos negativos clasificados por el modelo.
- Abajo a la derecha el porcentaje de verdaderos positivos que clasificó el modelo.

Entiéndase como positivo (1) una persona que cumplió con sus obligaciones financieras  y negativo (0) una persona que incumplió éstas.
Con esto en mente, se puede ver que el modelo ajustado es muy bueno para predecir verdaderos positivos, pero no para verdaderos negativos que los termina pasando como falsos positivos, lo que en contexto significaría otorgar créditos a muchas personas.

![Curva ROC](fig_m2.png){#fig-roc-curve-1}


En @fig-roc-curve-1 se puede observar la curva ROC donde se tiene un 77% de efectividad de clasificación, es decir, de que el modelo prediga que a una persona le acepten o le otorguen el crédito cuando en realidad cumple las condiciones y que se lo rechacen cuando no las cumple.
